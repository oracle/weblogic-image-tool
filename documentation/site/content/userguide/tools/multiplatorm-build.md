...
6. Completely disabled selinux does not seems to resolve issue completely,  check also /var/log/messages  , /var/log/audit/audit.log 
7. Whenever it is stuck ,  ps axww | grep qemu on the build machine shows two identical java processes, killing the last one will proceed most of time
but may become stuck again.  Thread dumps shows it is stuck in RuntimeExec internal class, it looks like the installer is spawning
multiple threads and making native os calls when expanding files during FMW install,  I suspect podman emulation may not
handle the multi thread context correctly.  Note: kill -3 on the second process produces no thread dump - indicates it may be
a thread or sub process by itself.  Interestingly it doesn't happen that much when using docker. 
---
