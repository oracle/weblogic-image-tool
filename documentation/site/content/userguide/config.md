...
- `WLSIMG_BUILDER` - As an alternative to the command-line argument `--builder`, this variable can be used to override the tool to process the Dockerfile (e.g.,`docker` or `podman`). The provided value should be the full path to the executable. For example, WLSIMG_BUILDER="/usr/bin/docker".
